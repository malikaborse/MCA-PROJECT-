    ...
    return (
        <>
            <div className='mx-5 mb-10 md:mx-16'>
                <h3 className='mb-9 md:text-2xl font-mono font-semibold text-lg text-center'>- Explore Our Intriors -</h3>
                    ...
                    {menu_list.map((item, index) => {
                        return (
                            <div onClick={() => setCategory(prev => prev === item.menu_name ? "All" : item.menu_name)} key={index} className='text-center hover:cursor-pointer'>
                                <img src={item.menu_image} style={{ maxWidth: '120px', maxHeight: '120px' }} className={category === item.menu_name ? "border-[3px] rounded-full border-blue-400" : "rounded-full"} />
